<template>
  <div>
    <el-row style="margin-bottom: 10px">
      <el-col :span="12">
        <el-breadcrumb-item>
          <el-button size="medium" type="danger" round>普通用户管理</el-button>
        </el-breadcrumb-item>
      </el-col>
  ...
  export default {
    ...
    methods: {
      deletePeople(rowData) {
        transferData.deletePeople(rowData.id)
          .then(res => {
            this.successResult();
            this.findAllPeoples();
          })
      ...
      findAllPeoples(page) {
        ...
        transferData.findAllPeople(page).then(res => {
      ...
      saveInfo() {
        ...
        //上传信息之后
        transferData.savePeople(this.editForm).then(res => {
          utils.sucess("保存成功");
          this.findAllPeoples();
        })
      },
      ...
      //搜索内容
      searchContent() {
        transferData.searchPeople(this.searchForm).then(res => {
